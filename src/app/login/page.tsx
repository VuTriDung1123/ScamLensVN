  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (err.code === "auth/unauthorized-domain") {
        ...
        setError(`Tên miền chưa được cấp phép trong Firebase! Vui lòng vào Firebase Console > Authentication > Settings > Authorized domains và thêm domain: "${currentDomain}"`);
      } else if (err.code === "auth/popup-closed-by-user" || err.code === "auth/cancelled-popup-request") {
        setError("Bạn đã đóng cửa sổ đăng nhập trước khi hoàn tất.");
      } else if (err.code === "auth/popup-blocked") {
        setError("Trình duyệt đã chặn cửa sổ bật lên (popup). Vui lòng cho phép popup hoặc mở web trong tab mới.");
      } else if (err.code === "auth/invalid-api-key" || err.code === "auth/api-key-not-valid") {
        setError("Khóa API Firebase không hợp lệ hoặc chưa được cấu hình biến môi trường NEXT_PUBLIC_FIREBASE_API_KEY.");
      } else if (err.code === "auth/operation-not-allowed") {
        setError("Tính năng đăng nhập Google chưa được kích hoạt trong Firebase Console (Authentication > Sign-in method > Google > Enable).");
      } else {
        setError(`Đăng nhập Google thất bại: ${err.message || err.code || "Lỗi không xác định"}`);
      }
      setLoading(false);
    }
  };
