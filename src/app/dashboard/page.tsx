...
export default function Dashboard() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      console.error(err);
      const serverMsg = err.response?.data?.error || err.message || "Vui lòng kiểm tra lại kết nối hoặc API backend.";
      setError(`Có lỗi xảy ra khi phân tích: ${serverMsg}`);
    } finally {
      setLoading(false);
    }
  };
          ...
          >
              ...
              <div className="bg-slate-900 border border-slate-700 rounded-3xl p-8 shadow-xl mt-8">
                ...
                {hasClicked === null ? (
                  ...
                  <div className="bg-emerald-950/30 border border-emerald-500/30 rounded-2xl p-6 transition-all">
                    ...
                    <button onClick={() => setHasClicked(null)} className="mt-6 text-sm text-slate-400 underline hover:text-white cursor-pointer">Thay đổi tình trạng</button>
                  </div>
                ) : (
                  <div className="bg-rose-950/30 border border-rose-500/30 rounded-2xl p-6 transition-all">
                    <div className="flex items-center gap-3 mb-4">
